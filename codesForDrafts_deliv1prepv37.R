...
del2Draft2= base2 + geom_histogram(binwidth = 1, aes(x= length)) + labs(title = "Distribution of Common Surnames", x = "Proportion per 100,000", y = "Number within Top 10 Names") + theme(plot.title = element_text(size=10)) +  annotate(geom = 'text', size = 2.75, label = note, y = -0.01, x = 10, angle = 0)
#Question: It's defaulting to x-axis starting at 5, since this is the lowest observed value. Is this considered deceptive like it would be if that happened with the x-axis?
del2Draft2
