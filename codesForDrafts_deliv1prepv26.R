...
del1Draft3 = del1Draft3 + theme(axis.text.x = element_text(angle = 90)) + labs(title = "10 Most Common Surnames in the United States", x = "Surname", y = "Frequency in USA, 2010") + theme(plot.title = element_text(size = 8.5))
del1Draft3
...
# save del1Draft ----------------------------------------------------------
saveRDS(del1Draft3, file = "del1Draft5.rds")


# deliverable 2 ----------------------------------------------------------

base2= ggplot(data=namedata2, aes(x= prop100k)) 
del2Draft2= base2 + geom_histogram(binwidth = 5, aes(x= prop100k)) + labs(title = "Distribution of Common Surnames", x = "Proportion per 100,000", y = "Number within Top 10 Names") + theme(plot.title = element_text(size=8.5)) + scale_x_reverse()
del2Draft2
#This shows greater clustering of names the further down the list you go - i.e. a steep slope down. It feels like this one would be better with more than the top 10, and in reverse order if only it would let me. Maybe I can format it with 2 axes to permit this?
